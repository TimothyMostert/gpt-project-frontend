  ...
  <div class="grid grid-cols-5 gap-4 h-24 w-full ">
    ...
    <div class="col-span-3 flex flex-col gap-2">
      ...
      <div class="flex flex-none items-center gap-x-4">
        <a
          @click="setTrip"
          class="rounded-md bg-gradient-to-r from-primaryOrange to-secondaryOrange px-2.5 py-1.5 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50"
          >View trip<span class="sr-only">, {{ trip.title }}</span></a
        >
        <svg v-if="!loading"  @click="deleteTrip" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
          <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
        </svg>
        <svg v-else xmlns="http://www.w3.org/2000/svg" class="animate-spin -ml-1 mr-3 h-5 w-5 text-gray-500" viewBox="0 0 24 24" fill="currentColor">
...
const image = computed(() => {
  if (props.trip.main_photo) {
    return `https://maps.googleapis.com/maps/api/place/photo?maxwidth=200&photo_reference=${props.trip.main_photo}&key=${import.meta.env.VITE_APP_GOOGLE_MAPS_API_KEY}`;
  } else {
